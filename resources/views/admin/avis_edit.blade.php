<x-app-layout>
    ...
    <div class="py-12">
        ...
        @endif
        <form action="/avis/{{ $avi->id }}" method="POST" class="w-full max-w-xl bg-white rounded-lg px-4 p-2 mx-auto">
            @csrf
            @method('put')
            <div class="flex flex-wrap -mx-3 mb-6">
                <h2 class="px-4 pt-3 pb-2 text-gray-800 text-lg">Modifier un avis</h2>
                <div class="w-full md:w-full px-3 mb-2 mt-2">
                    <input
                        ...
                        name="auteur" placeholder="L'auteur" value="{{ $avi->auteur }}" required/>
                </div>
                <div class="w-full md:w-full px-3 mb-2 mt-2">
                    <textarea
                        class="bg-gray-100 rounded border border-gray-400 leading-normal resize-none w-full h-20 py-2 px-3 font-medium placeholder-gray-700 focus:outline-none focus:bg-white"
                        name="texte" placeholder="Ecrire le texte de l'avis" required>{{ $avi->texte }}</textarea>
                </div>
